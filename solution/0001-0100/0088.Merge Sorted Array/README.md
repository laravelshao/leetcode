...
具体地，我们用两个指针 `len1 = m - 1` 和 `len2 = n - 1` 分别指向两个数组的末尾，用一个指针 `len = m + n - 1` 指向合并后的数组的末尾。每次比较两个数组的末尾元素，将较大的元素放在合并后的数组的末尾，然后将指针向前移动一位，重复这个过程，直到其中1个数组遍历完。

如果 `nums2` 数组还剩下元素，则需要填充到数字前面位置。
