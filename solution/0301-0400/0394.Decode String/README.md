...
用栈 stack_multi 存储左括号前的数字 multi，栈 stack_res 存储左括号前的字符串 res。

遍历字符串 s 中每个字符 c：

-   若 c 是数字，则累乘数字 multi
-   若 `c == '['`，则将左括号前的数字 multi 存入 stack_multi，左括号前的字符串 res 存入 stack_res，并将 multi 重新置为 0，res 置为空串
-   若 `c == ']'`，则 `res = stack_res.removeLast() + res 重复 stack_multi.removeLast() 次数`
-   若 c 是字符，则累加字符串 res
